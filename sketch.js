// ===================================
// トイレダッシュ！迷路でうんち回収ゲーム
//         - 完成版 (p5.js内で名前入力・バグ修正版) -
// ===================================

// ★DREAMLOのキーとURLを設定
const DREAMLO_PRIVATE_CODE = 'D6HHeTkZtUqphBGdMlM3nQ0PESB9HZhkSNVoji75S8sQ'; // スコア登録用
const DREAMLO_PUBLIC_CODE = '686ceabc8f40bc109c19a3b8';  // スコア取得用

// ★タイムが短い順(昇順)でランキングを扱うためのURLに変更
const DREAMLO_URL_ADD = `https://www.dreamlo.com/lb/${DREAMLO_PRIVATE_CODE}`; // httpPostで使うベースURL
const DREAMLO_URL_GET = `https://www.dreamlo.com/lb/${DREAMLO_PUBLIC_CODE}/json-asc/10`; // 上位10件を昇順で取得
...
// ゲームの状態管理
let gameState = 'start'; // 'start', 'playing', 'clear', 'naming', 'submitting', 'ranking'
...
function drawNamingScreen() {
    ...
    fill(WALL_COLOR);
    textSize(30);
    text(`タイム: ${clearTime.toFixed(2)} 秒`, width / 2, height / 2 - 150);

    textSize(40);
    text('名前を入力してください', width / 2, height / 2 - 50);
    ...
    rectMode(CENTER);
    fill(PATH_COLOR);
    rect(width / 2, height / 2 + 30, 400, 60, 10);
    ...
    text(displayName, width / 2 - 180, height / 2 + 30);
    rectMode(CORNER);

    textAlign(CENTER, CENTER);
    textSize(20);
    text('Enterキーで登録 | Escキーでタイトルへ', width/2, height/2 + 120);
}

function drawSubmittingScreen() {
    background(PATH_COLOR);
    fill(WALL_COLOR);
    textAlign(CENTER, CENTER);
    textSize(50);
    text('スコア登録中...', width / 2, height / 2);
}
...
function keyPressed() {
    ...
    if (gameState === 'naming') {
        if (keyCode === ENTER) {
            ...
            gameState = 'start';
            initGame();
        } else if ((keyCode >= 65 && keyCode <= 90) || (keyCode >= 48 && keyCode <= 57)) { // A-Z, 0-9
...
function submitScore() {
    ...
    const score = floor(clearTime * 100); 

    const url = `${DREAMLO_URL_ADD}/add-pipe`;
    const data = `${encodeURIComponent(playerName)}|${score}`;
    
    httpPost(url, 'text', data, (response) => {
